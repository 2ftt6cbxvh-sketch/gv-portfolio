...
/**
 * Instant IP Unblocker via Telegram (/unblock <IP>)
 * Removes the specified IP address from the 24-hour rate limit block list in PostgreSQL.
 */
export async function unblockIp(targetIp) {
  try {
    const flag = await prisma.featureFlag.findUnique({
      where: { key: "ip_rate_limits_24h" },
    });

    if (!flag?.metadata) return false;

    let records = typeof flag.metadata === "string" ? JSON.parse(flag.metadata) : flag.metadata;
    if (records[targetIp]) {
      delete records[targetIp];
      await prisma.featureFlag.upsert({
        where: { key: "ip_rate_limits_24h" },
        update: { metadata: JSON.stringify(records) },
        create: { key: "ip_rate_limits_24h", name: "24h IP Rate Limits", enabled: true, metadata: JSON.stringify(records) },
