...
export async function getThemeTokens() {
  ...
  if (!map["landing.logoColor"]) map["landing.logoColor"] = "#00f0ff";
  if (!map["landing.sparkColor"]) map["landing.sparkColor"] = "#00ffff";
  if (!map["editor.accent"]) map["editor.accent"] = "#a56ce8";
  if (!map["analyst.accent"]) map["analyst.accent"] = "#ff0f06";
  if (!map["developer.accent"]) map["developer.accent"] = "#fffc00";
  return map;
}
...
export async function getPublicSiteData() {
  const [settings, modeRows, themeTokens] = await Promise.all([
    ...
    prisma.modeContent.findMany({
      ...
      include: {
        ...
        education: { where: { visible: true }, orderBy: { order: "asc" } },
      },
    }),
    getThemeTokens(),
  ]);
  ...
  if (settings.twitterUrl) social.push({ label: "Instagram", href: settings.twitterUrl });
  ...
  // Fetch initial killswitch, maintenance, and landing aesthetic flags for instant 0ms SSR rendering
  const [killFlag, maintFlag, aestheticFlag] = await Promise.all([
    prisma.featureFlag.findUnique({ where: { key: "emergency_killswitch" } }).catch(() => null),
    prisma.featureFlag.findUnique({ where: { key: "under_maintenance_mode" } }).catch(() => null),
    prisma.featureFlag.findUnique({ where: { key: "landing_aesthetic" } }).catch(() => null),
  ]);

  let initialKillswitch = !!killFlag?.enabled;
  let initialMaintenance = { active: !!maintFlag?.enabled, metadata: null };
  if (maintFlag?.metadata) {
    try {
      initialMaintenance.metadata = typeof maintFlag.metadata === "string" ? JSON.parse(maintFlag.metadata) : maintFlag.metadata;
    } catch (e) {}
  }

  let initialAesthetic = "glass";
  if (aestheticFlag?.metadata) {
    let raw = String(aestheticFlag.metadata).trim().toLowerCase().replace(/^"|"$/g, '');
    try {
      const parsed = JSON.parse(raw);
      if (typeof parsed === "string") raw = parsed.trim().toLowerCase();
      else if (parsed?.key) raw = String(parsed.key).trim().toLowerCase();
      else if (parsed?.mode) raw = String(parsed.mode).trim().toLowerCase();
    } catch (e) {}
    if (["liquid", "glass", "aero", "classic"].includes(raw)) {
      initialAesthetic = raw;
    }
  }
